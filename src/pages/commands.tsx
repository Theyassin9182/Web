...
export default function CommandsPage({ user }: PageProps) {
	...
	Object.values(commandsData).forEach((commands) => allCommands.push(...commands));
	...
	useEffect(() => {
		...
		setCommands(
			currentCategory.startsWith("all")
				? allCommands.filter(
						(command) =>
							command.t.some((x) => x.includes(search.toLowerCase())) ||
	...
	return (
		<Container title="Commands" user={user}>
			...
			<div className="relative my-20 flex flex-col space-y-8">
				<div>
					<div className="font-montserrat text-4xl font-bold text-dank-200 dark:text-white">COMMANDS</div>
					...
					<div className="max-w-3xl text-lg  text-light-600 dark:text-light-300">
						Find all the information, including required permissions, regarding the extensive list of
				...
				<div className="flex items-center space-x-0 lg:space-x-4">
					...
					<div className="pl-4 lg:pl-0">
						<Searchbox placeholder="Search for a command" setSearch={setSearch} />
					</div>
				</div>
				<div className="flex cursor-pointer flex-col space-y-4">
					{commands?.map((command) => (
						<Expandable
							name={`${command.t[0]}${command.pS || command.dO ? " ⭐" : ""}`}
